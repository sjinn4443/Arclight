document.addEventListener("DOMContentLoaded", function () {
  ...
  function checkTopSectionCompletion() {
    const onsetSelected = document.querySelector(
  ...
  const topInputs = document.querySelectorAll(
  ...
  fundalButtons.forEach((button) => {
    button.addEventListener("click", function () {
      ...
      if (val === "normal") {
        ...
        const poorViewBtn = document.querySelector(
          '.back-btn[data-value="poor view"]',
        );
        if (poorViewBtn) {
  ...
  backButtons.forEach((button) => {
    button.addEventListener("click", function () {
      if (
        document.getElementById("back-section").classList.contains("disabled")
      ) {
        return;
      }
      backButtons.forEach((btn) => {
  ...
  function calculateResult() {
    ...
    const onsetElem = document.querySelector(
      '#top-section input[name="onset"]:checked',
    );
    if (!onsetElem) {
    ...
    const nearVAValue = document
      .getElementById("nearVA")
      .value.trim()
      .toUpperCase();
    ...
    const eyesElem = document.querySelector(
      '#top-section input[name="eyes"]:checked',
    );
    ...
    if (eyes === "one" && onsetValue === "sudden" && painYes) {
      referral =
        "YES (Refer urgently for surgery) - Sudden onset, pain, ?acute glaucoma/other";
      if (cataractType !== "Nil") {
    ...
    const pupilSelected = document.querySelector(
      '#top-section input[name="pupil"]:checked',
    );
    ...
    const oneEyeStatus =
      document.querySelector('#top-section input[name="eyes"]:checked')
        ?.value === "one";
