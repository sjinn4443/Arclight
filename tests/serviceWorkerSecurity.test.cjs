...
function loadServiceWorker(fetchImpl) {
  ...
  const caches = {
    ...
    keys: jest
      .fn()
      .mockResolvedValue(["arclight-static-v19", "arclight-static-v20"]),
...
describe("service worker sensitive-cache policy", () => {
  ...
  test("removes the previous cache version during activation", async () => {
    ...
    expect(caches.delete).toHaveBeenCalledWith("arclight-static-v19");
    expect(caches.delete).not.toHaveBeenCalledWith("arclight-static-v20");
