...
const baseline = execFileSync('C:/Program Files/Git/cmd/git.exe', ['show', 'HEAD:public/js/i18n.js'], {encoding:'utf8'});
...
async function check(source, label) {
  ...
  const w = dom.window;
  w.fetch = async url => ({ok:true, json:async () => JSON.parse(fs.readFileSync('public/' + String(url).replace(/^\//, ''), 'utf8'))});
  w.eval(media.replace(/export /g, '') + '\n' + source.replace(/^import .*;\r?\n/gm, '').replace(/export /g, ''));
  const block = w.document.getElementById('practiceLevelTemplate').content.firstElementChild.cloneNode(true);
  ...
  observer.observe(w.document.body, {childList:true, characterData:true, subtree:true});
  await delay(350);
