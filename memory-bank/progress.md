...
- Bumped the service-worker static cache to `arclight-static-v34` so installed clients receive the Fundal Reflex examination narration controls and current HTML/CSS/JS fragments.
...
- Lao localization: `lo` is supported across the main locale dictionary, language/install flow, Medical Students workshop, shared mini-app strings, Fundal Reflex, dynamic notices/quiz feedback/settings/My Learning/case-study copy, accessible labels, and supported app/Childhood video subtitle VTT/HLS assets.
- Lao translation hardening: literal translation now cleans a targeted set of embedded English clinical terms, newly inserted My Learning metadata preserves i18n keys, settings uses localized in-app confirmation dialogs, and translation completeness/terminology tests cover the expanded Lao key set.
- Case/navigation refinement: intermediate case studies provide history-backed Previous/Next controls without counting a case twice, primary flashcards accept desktop mouse/pen swipes as well as touch, and shared Visual Acuity video pages remember their actual launcher for contextual Back navigation.
...
- Translation QA is clean as of `2026-08-28`, but new copy can regress it if locale keys, mini-app dictionaries, VTT files, generated HLS subtitles, or subtitle catalogs are not updated with the feature change.
