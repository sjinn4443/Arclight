...
Create a dedicated PostgreSQL role with `CONNECT`, schema `USAGE` and `SELECT` on `app_users`, `ip_logs`, `app_users_latest_first` and `ip_logs_latest_first`. It must have no write, DDL, role, replication, bypass-RLS or superuser rights. Store its connection string only as `REPORTS_READ_DATABASE_URL` in Railway.

Set these variables on the reporting service:

| Variable | Purpose |
|---|---|
| `REPORTS_READ_DATABASE_URL` | Dedicated read-only PostgreSQL connection |
